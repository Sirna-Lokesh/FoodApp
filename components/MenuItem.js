  ...
  return (
    <div id="menu-item-container" className="bg-gray-200 bg-opacity-50  flex flex-wrap w-[500px] h-[150px] border border-black m-2 justify-between">
      <div id="menu-item-info" className="mx-3 my-4">
        ...
        <h3 className="font-bold">Price - {parseInt(price) / 100}</h3>
        <h3 className="font-bold">Rating - {avgRating==null ? <span>No Rating </span> :<button
          id="rating-btn"
        ...
        <button data-testid="add-item-button" className="bg-blue-300 m-1 p-1 border border-black" onClick={()=>handleAddItems({imageId,price,name,avgRating,category})}> Add-Item </button>
      </div>
